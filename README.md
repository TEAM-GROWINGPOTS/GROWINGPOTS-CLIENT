...
<table>
  <tr>
    <td align="center" width="25%">
      <h3>🚀 새로운 시도</h3>
      <p>익숙한 선택 대신<br/>더 나은 방향을 탐구해요.<br/>기술 도입도, 구조 설계도<br/>근거 있으면 환영해요.</p>
    </td>
    <td align="center" width="25%">
      <h3>🤝 함께 풀기</h3>
      <p>혼자 고민하지 않아요.<br/>빠른 질문, 즉시 리뷰,<br/>지식 공유로 팀 전체가<br/>함께 성장해요.</p>
    </td>
    <td align="center" width="25%">
      <h3>🔥 끝까지 파고들기</h3>
      <p>막히면 포기하지 않아요.<br/>에러 하나도 원인까지 추적하고<br/>해결 과정을 기록으로<br/>남겨요.</p>
    </td>
    <td align="center" width="25%">
      <h3>✨ 야무진 마무리</h3>
      <p>동작만이 목표가 아니에요.<br/>코드 품질, 타입 안정성,<br/>문서화까지 끝까지<br/>책임져요.</p>
    </td>
  </tr>
</table>
...
Next.js App Router + features/shared 2계층 구조를 기반으로 해요.

```
└── src/
    ├── app/                    # Next.js App Router — 페이지별 라우팅
    │   ├── (auth)/
    │   ├── (main)/
    │   └── (onboarding)/
    ├── features/               # 페이지·기능별 도메인 컴포넌트
    │   ├── onboarding/
    │   ├── home/
    │   ├── card-view/
    │   └── node-view/
    └── shared/                 # 전 레이어에서 쓰는 범용 공통
        ├── apis/
        ├── components/
        ├── constants/
        ├── hooks/
...
`type/feature-name/#issue-number` 형식으로 관리해요. (kebab-case)

```
init/project-setup/#1
feat/login-page/#12
fix/sidebar-item/#34
refactor/api-module/#56
...
<details>
<summary><b>🧩 컴포넌트</b></summary>
<br/>

**네이밍**

- 리액트 컴포넌트 → `PascalCase` (예: `Header`)
- 유틸 / 함수 / 변수 → `camelCase`

**TSX**

- 의미 없는 `div` 남발 지양 → 시맨틱 태그 우선 (`section`, `header`, `main` 등)
- 최상단 래퍼가 의미 없으면 Fragment(`<>...</>`) 사용
- children이 없는 컴포넌트는 self closing: `<EmptyState />`
...
<details>
<summary><b>⛓️ 타입</b></summary>
<br/>

- 타입 / 인터페이스 이름은 `PascalCase`
- 기본적으로 `type`보다 `interface` 우선
...
- **질문에 진입장벽 없기** — 모르는 건 바로 물어보고, 아는 건 바로 공유해요
- **PR은 미루지 않기** — 올라오는 즉시 리뷰해서 흐름이 끊기지 않게 해요
- **사람이 먼저** — 코드보다 서로에 대한 존중이 우선이에요

<br/>

<div align="center">

질문하는 용기 · 공유하는 문화 · 문서화하는 습관을 지켜가요
